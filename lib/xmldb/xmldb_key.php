...
class xmldb_key extends xmldb_object {
    ...
    /** @var string onupdate action */
    protected $onupdate;
    
    /** @var array allowed value for foreign key */
    private $foreignkeyvalues = array(null,"noaction","cascade","restrict","set null","set default");
     ...
     * @param string $ondelete one of :  null,"noaction","cascade","restrict","set null","set default"
     * @param string $onupdate one of :  null,"noaction","cascade","restrict","set null","set default"
     */
    ...
    public function set_attributes($type, $fields, $reftable=null, $reffields=null, $ondelete=null, $onupdate=null) {
        ...
        $this->ondelete = (array_search($ondelete,$this->foreignkeyvalues)) ? $ondelete : null;
        $this->onupdate = (array_search($onupdate,$this->foreignkeyvalues)) ? $onupdate : null;
    }
    ...
    public function arr2xmldb_key($xmlarr) {
        ...
        if (isset($xmlarr['@']['ONDELETE'])) {
            if ($this->type == XMLDB_KEY_FOREIGN ||  $this->type == XMLDB_KEY_FOREIGN_UNIQUE) {
                ...
                if (!array_search($ondelete,$this->foreignkeyvalues)) {
         ...
         if (isset($xmlarr['@']['ONUPDATE'])) {
             if ($this->type == XMLDB_KEY_FOREIGN ||  $this->type == XMLDB_KEY_FOREIGN_UNIQUE) {
                 $onupdate = strtolower(trim($xmlarr['@']['ONUPDATE']));
                 if (!array_search($onupdate,$this->foreignkeyvalues)) {
                         $this->errormsg = 'Wrong ONUPDATE attribute :"'.$onupdate.'"(valid options : "noaction","cascade","restrict","set null","set default")';
                         $this->debug($this->errormsg);
                         $result = false;
                     }
             } else {
                 $this->errormsg = 'Wrong ONUPDATE attribute (only FK can have it)';
                 $this->debug($this->errormsg);
                 $result = false;
             }
         }
